...
const showHeader = computed(
    () => authUser.isAuthenticated && !route.meta.public,
);
...
<template>
    <header v-if="showHeader" class="border-b border-slate-200 bg-white">
        <div class="mx-auto flex max-w-3xl items-center justify-between p-4">
            <RouterLink to="/ladders" class="font-semibold text-slate-900"
                >Tennis Ladder</RouterLink
            >
            <div class="flex items-center gap-3">
                <span class="text-sm text-slate-500">{{ displayName }}</span>
                <BaseButton variant="secondary" @click="onSignOut"
                    >Sign out</BaseButton
                >
            </div>
        </div>

...
<template>
    <ul class="flex flex-col gap-2">
        ...
        >
            <form v-if="editingId === player.id" class="flex flex-1 items-end gap-2" @submit.prevent="save(player.id)">
            ...
            <template v-else>
                ...
                <div class="flex gap-2">
                    <BaseButton variant="secondary" @click="startEditing(player)">Rename</BaseButton>
                    <BaseButton variant="danger" @click="emit('remove', player.id)">Delete</BaseButton>
                </div>
            </template>
        </li>
    </ul>
